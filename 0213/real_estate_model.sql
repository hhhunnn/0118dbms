...
FROM qna ORDER BY reception_number DESC;

-- 8. 관리자로 미완료 게시물 보기
SELECT reception_number, reply_status, title, writer_id, write_datetime, view_count
FROM qna WHERE reply_status = 0
ORDER BY reception_number DESC;

-- 9. 검색 게시물 목록 보기 
SELECT reception_number, reply_status, title, writer_id, write_datetime, view_count
FROM qna WHERE title LIKE '%질문%' ORDER BY reception_number DESC;

-- 10. 특정 게시물 보기
SELECT title, writer_id, write_datetime, view_count, contents, reply
FROM qna WHERE reception_number = 1;

-- 11. 관리자로 답변 작성 (관리자ID를 'userID'라고 지음 / 원래는 userID로 지정하면 안된다)
