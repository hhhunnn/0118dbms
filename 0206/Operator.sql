...
SELECT above_avg_spd, above_avg_dir, null_column1, null_column2 FROM practice_sql.jeju;

UPDATE jeju SET above_avg_spd = true
WHERE speed_80m > 5.5;

UPDATE jeju SET above_avg_spd = false
WHERE speed_80m <= 5.5;

UPDATE jeju SET above_avg_dir = true
WHERE direction_50m > 160;

UPDATE jeju SET above_avg_dir = false
WHERE direction_50m <= 160;

UPDATE jeju SET null_column1 = speed_80m
WHERE speed_80m > 5.5;

UPDATE jeju SET null_column2 = direction_50m
WHERE direction_50m > 160;
...
SELECT observe_date FROM jeju ORDER BY speed_80m DESC; -- 가능
